...
train_stage=-10
get_egs_stage=-10
dir=exp/chain/tdnn_1a_fbank  # Note: _sp will get added to this
decode_iter=
...
if [ $stage -le 6 ]; then
    #utils/copy_data_dir.sh data/${train_set}_all  data/${train_set}_all_hires
  # Although the nnet will be trained by high resolution data, we still have to
  # perturb the normal data to get the alignment _sp stands for speed-perturbed
    echo "$0: preparing directory for low-resolution speed-perturbed data (for alignment)"
    #utils/data/perturb_data_dir_speed_3way.sh data/${train_set} data/${train_set}_sp
    echo "$0: making Fbank features for low-resolution speed-perturbed data"
    utils/copy_data_dir.sh data/${train_set} data/${train_set}_fbank
    steps/make_fbank.sh --cmd "$train_cmd" \
    --nj 70  data/${train_set}_fbank exp/make_mfcc/train_fbank mfcc_fbank || exit 1;
    steps/compute_cmvn_stats.sh data/${train_set}_fbank \
    exp/make_mfcc/train_fbank mfcc_fbank || exit 1;
...
if [ $stage -le 11 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --feat-dir data/${train_set}_fbank \
    --tree-dir $treedir \
